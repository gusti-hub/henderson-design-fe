...
const AdminPanel = () => {
  ...
  const handleMenuClick = (path, onClick) => {
    if (onClick) { onClick(); } else { setActiveMenu(path); }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-[#005670] to-[#007a9a] shadow-lg">
        <div className="flex items-center justify-between p-4">
          <img src="/images/HDG-Logo.png" alt="Henderson Design Group" className="h-8 w-auto brightness-0 invert" />
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 text-white hover:bg-white/10 rounded-lg transition-colors">
        ...
        `}>
          ...
          <div className="p-6 border-b border-white/10">
            <div className="flex items-center gap-3 mb-2">
              <img src="/images/HDG-Logo.png" alt="Henderson Design Group" className="h-10 w-auto brightness-0 invert" />
            </div>
          ...
          <nav className="flex-1 overflow-y-auto py-4 px-3">
            <div className="space-y-1">
              {menuItems.map((item, index) => {
                ...
                return (
                  <button key={index} onClick={() => handleMenuClick(item.path, item.onClick)}
                    className={`w-full group relative overflow-hidden rounded-xl transition-all duration-200
                      ${isActive ? 'bg-white text-[#005670] shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}>
                    <div className="flex items-center gap-3 p-3">
                      {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-[#005670] to-[#007a9a] rounded-r-full" />}
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors duration-200
                        ${isActive ? 'bg-gradient-to-br from-[#005670] to-[#007a9a]' : 'bg-white/5 group-hover:bg-white/10'}`}>
                        <item.icon className={`w-5 h-5 ${isActive ? 'text-white' : ''}`} />
                      </div>
                      <div className="flex-1 text-left">
                        <p className={`text-sm font-semibold ${isActive ? '' : 'group-hover:text-white'}`}>{item.label}</p>
                        <p className={`text-xs ${isActive ? 'text-[#005670]/60' : 'text-white/40 group-hover:text-white/60'}`}>{item.description}</p>
                      </div>
                      {isActive && <ChevronRight className="w-4 h-4 text-[#005670]" />}
                    </div>
                  </button>
                );
              })}
                ...
                return (
                  <button key={index} onClick={() => handleMenuClick(item.path, item.onClick)}
                    ...
                    <div className="flex items-center gap-3 p-3">
                      ...
                      {item.path === '/logout' && <ChevronRight className={`w-4 h-4 ${isActive ? 'text-[#005670]' : 'text-white/60'}`} />}
                    </div>
                  </button>
                );
              })}
            ...
            <div className="px-6 py-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">Henderson Design Group Admin Portal</p>
                </div>
              </div>
            </div>
          </header>
