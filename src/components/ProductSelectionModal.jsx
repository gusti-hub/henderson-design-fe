...
import { X, Search, Loader2, Check, ChevronLeft, ChevronRight, ExternalLink, LayoutGrid, List } from 'lucide-react';
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                <button
                  ...
                  className={`p-1.5 transition-colors ${!listView ? 'bg-[#005670] text-white' : 'text-gray-500 hover:bg-gray-100'}`}
                  title="Grid view"
                >
                  ...
                  {filteredProducts.map(product => {
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-gray-100 cursor-pointer transition-colors ${isSelected ? 'bg-[#005670]/5' : 'hover:bg-gray-50'}`}
                      >
                        <td className="pl-3 pr-1 py-2">
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${isSelected ? 'bg-[#005670] border-[#005670]' : 'border-gray-300'}`}>
                        ...
                        <td className="px-3 py-2">
                          {product.package && (
                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                              product.package === 'Lani' ? 'bg-emerald-100 text-emerald-800' : 'bg-violet-100 text-violet-800'
                            }`}>{product.package}</span>
                          )}
                        </td>
                        <td className="px-3 py-2 text-xs text-gray-600 whitespace-nowrap">{buyPrice > 0 ? `$${buyPrice.toFixed(2)}` : '—'}</td>
                        <td className="px-3 py-2 text-xs font-semibold text-[#005670] whitespace-nowrap">{sellPrice > 0 ? `$${sellPrice.toFixed(2)}` : '—'}</td>
                        <td className="px-3 py-2">
                          <button
                            onClick={e => { e.stopPropagation(); setDetailProduct(product); }}
                            className="p-1 text-gray-400 hover:text-[#005670] transition-colors"
                            title="View details"
                          >
