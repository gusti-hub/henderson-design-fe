...
const ROOM_OPTIONS = [
  ...
  'DEN','WET BAR','MEDIA ROOM','HALLWAY 1','HALLWAY 2','LANAI','LANAI 1','LANAI 2','LANAI 3','MAIN LANAI','BBQ AREA',
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => { setLocal('itemClass', e.target.value); upd('itemClass', e.target.value); }}
                      className={`${inputCls} bg-white`}
                    >
                      <option value="">Select item type…</option>
                      {ITEM_TYPE_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
                      {localFields.itemClass && !ITEM_TYPE_OPTIONS.includes(localFields.itemClass) && (
                        <option value={localFields.itemClass}>{localFields.itemClass}</option>
                      )}
                    </select>
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updCA('availabilityStatus', e.target.value)}
                      className={`${inputCls} bg-white`}
                    >
                      ...
                      {CATEGORY_SPECIFIC_FIELDS[localFields.itemClass].map(({ k, l, bool, num, sel }) => (
                        <div key={k}>
                          ...
                          {bool ? (
                            ...
                            <select
                              value={customAttrs[k] || ''}
                              onChange={(e) => updCA(k, e.target.value)}
                              className={`${inputCls} bg-white`}
                            >
                              <option value="">Select…</option>
                              {sel.map(o => <option key={o} value={o}>{o}</option>)}
                            </select>
                          ) : (
                            <input
